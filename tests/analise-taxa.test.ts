...
test("análise da taxa: cotas − contratos − manutenção + acordo = resultado 3", () => {
  const lancamentos = [
  ...
  assert.equal(r.resultado1Cents, 50_000_00);
  assert.equal(r.manutencoesCents, 20_000_00);
  assert.equal(r.resultado2Cents, 30_000_00);
  assert.equal(r.cotasAcordoCents, 5_000_00);
  assert.equal(r.resultado3Cents, 35_000_00);
});
